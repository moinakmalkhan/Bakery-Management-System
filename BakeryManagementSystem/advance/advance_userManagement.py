...
import time
class Advance_UserManagement(tk.Toplevel):
    """
    ...
    def __init__(self,win,*args, **kwargs):
        ...
        self.add_user_img = ImageTk.PhotoImage(Image.open('./images/add_user.png'))

        self.delete_img = ImageTk.PhotoImage(Image.open('./images/delete.png'))

        self.edit_img = ImageTk.PhotoImage(Image.open('./images/edit_user.png'))

        self.column_dict = {
            "User ID":"id","Joining Date":"joiningDate", "Full name":"name",
             "Username":"username", "Email":"email", "Address":"address", 
             "Phone":"phone", "CNIC":"cnic", "User Status":"status"
        }
    def userManagementGUI(self):
        ...
        ttk.OptionMenu(top_frame,self.column_to_search,"User ID",*headings).grid(column=2,row=1)
        ...
        ttk.OptionMenu(top_frame,self.orderby,"User ID",*headings).grid(column=6,row=1)
        ...
        self.column_to_search.trace('w',lambda *args: self.findUser())
        self.searchvalue.trace('w',lambda *args: self.findUser())
        self.operator.trace('w',lambda *args: self.findUser())
        self.order.trace('w',lambda *args: self.findUser())
        self.orderby.trace('w',lambda *args: self.findUser())
        ...
        self.tree.bind("<Delete>",lambda *arg: self.delete_user())
        self.tree.bind("<Double-Button-1>",lambda *arg: self.add_update_user_GUI(action="Update"))
    ...
    def findUser(self):
        ...
        query = F"""
        SELECT id,joiningDate,name,username,email,address,phone,cnic,status FROM user WHERE {self.column_dict[self.column_to_search.get()]} {operator_dict[self.operator.get()]} ORDER BY {self.column_dict[self.orderby.get()]} {self.order.get()}
        """
        c.execute(query)
        data = c.fetchall()
        if data:
    ...
    def add_user(self,e=None):
        ...
        if data:
            c.execute(f"INSERT INTO user(joiningDate,name,username,password,email,address,phone,cnic,status) VALUES{str(data)}")
            conn.commit()
